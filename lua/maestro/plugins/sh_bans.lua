...
maestro.command("unban", {"id", "reason:optional"}, function(caller, id, reason)
	maestro.unban(id, reason, caller and IsValid(caller) and caller:SteamID64())
	if reason then
		...
		q:Callback(function(res, status)
			if type(res) == "table" then
				...
				for i = 1, #res do
					...
					maestro.chatconsole(caller, col, "\t", os.date("%x - ", ban.when), "Ban ID #", ban.id, ": ", Color(255, 255, 255), ban.reason, col, " Length: ", Color(255, 255, 255), len)
				end
			end
		end)
	q:Execute()
